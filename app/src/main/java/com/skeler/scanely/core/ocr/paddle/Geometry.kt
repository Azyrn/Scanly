...
object Geometry {
    ...
    fun minAreaRect(pts: FloatArray): RotatedRect {
        ...
        if (n < 3) {
            ...
            for (i in 0 until n) {
                x0 = minOf(x0, hull[i * 2])
                x1 = maxOf(x1, hull[i * 2])
                y0 = minOf(y0, hull[i * 2 + 1])
                y1 = maxOf(y1, hull[i * 2 + 1])
            }
            return RotatedRect(
        ...
        for (i in 0 until n) {
            ...
            val ux = ex / len
            val uy = ey / len
            var p0 = Float.MAX_VALUE
            var p1 = -Float.MAX_VALUE
            var q0 = Float.MAX_VALUE
            var q1 = -Float.MAX_VALUE
            for (h in 0 until n) {
                val hx = hull[h * 2]
                val hy = hull[h * 2 + 1]
                val p = hx * ux + hy * uy
        ...
        val c0 = corner(bp0, bq0)
        val c1 = corner(bp1, bq0)
        val c2 = corner(bp1, bq1)
        val c3 = corner(bp0, bq1)
        return RotatedRect(
