...
is the query named "GetCurrentUser",
which can be accessed via the [ExampleConnector.getCurrentUser] property.


An example of the property for a mutation
is the mutation named "CreateFolder",
which can be accessed via the [ExampleConnector.createFolder] property.
...
For example, the "GetCurrentUser" query has no variables
and can be executed via the
[ExampleConnector.getCurrentUser]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getCurrentUser.execute()
println("GetCurrentUser query returned: ${queryResult.data}")
```
...
For example, the "ListDocumentsInFolder" query has 1 required variable ("folderId")
and can be executed via the [ExampleConnector.listDocumentsInFolder]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listDocumentsInFolder.execute(folderId=java.util.UUID.randomUUID())
println("ListDocumentsInFolder query returned: ${queryResult.data}")
```
...
For example, the "CreateFolder" mutation has 1 required variable ("name")
and can be executed via the [ExampleConnector.createFolder]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createFolder.execute(name="waldo")
println("CreateFolder mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateFolder" mutation has 2 optional variables ("description" and "parentFolderId")
and can be executed via the [ExampleConnector.createFolder]
property as follows:
...
val mutationResult = connector.createFolder.execute(name="waldo") {
  description = "baz"
  parentFolderId = java.util.UUID.randomUUID()
}
println("CreateFolder mutation returned: ${mutationResult.data}")
```
